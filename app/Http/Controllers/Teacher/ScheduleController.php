...
class ScheduleController extends Controller
{
    ...
    protected function getSessions(int $teacherId, string $tab)
    {
        ...
        return $query->get()->map(fn($booking) => [
            ...
            'date_key' => $booking->start_time->setTimezone(request()->user()?->timezone ?? config('app.display_timezone'))->format('Y-m-d'),
            'formatted_date' => $booking->start_time->setTimezone(request()->user()?->timezone ?? config('app.display_timezone'))->format('j M'),
            'formatted_day' => $booking->start_time->setTimezone(request()->user()?->timezone ?? config('app.display_timezone'))->format('j'),
            'formatted_month' => $booking->start_time->setTimezone(request()->user()?->timezone ?? config('app.display_timezone'))->format('F'),
            'formatted_start_time' => $booking->start_time->setTimezone(request()->user()?->timezone ?? config('app.display_timezone'))->format('g:i A'),
            'formatted_end_time' => $booking->end_time->setTimezone(request()->user()?->timezone ?? config('app.display_timezone'))->format('g:i A'),
