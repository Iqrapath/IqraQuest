...
class ScheduleController extends Controller
{
    ...
    protected function getSessions(int $teacherId, string $tab)
    {
        ...
        return $query->get()->map(fn($booking) => [
            ...
            'date_key' => $booking->start_time->setTimezone(request()->user()?->timezone ?? config('app.timezone'))->format('Y-m-d'),
            'formatted_date' => $booking->start_time->setTimezone(request()->user()?->timezone ?? config('app.timezone'))->format('j M'),
            'formatted_day' => $booking->start_time->setTimezone(request()->user()?->timezone ?? config('app.timezone'))->format('j'),
            'formatted_month' => $booking->start_time->setTimezone(request()->user()?->timezone ?? config('app.timezone'))->format('F'),
            'formatted_start_time' => $booking->start_time->setTimezone(request()->user()?->timezone ?? config('app.timezone'))->format('g:i A'),
