...
export default function SettingsIndex({ user, student, settings, currencies, mustVerifyEmail, requiresConfirmation }: Props) {
    ...
    return (
        <>
            ...
            <div className="max-w-[600px]">
                ...
                {/* Tab Content */}
                {activeTab === 'account' && (
                    <AccountInfoTab user={user} student={student} settings={settings} currencies={currencies} mustVerifyEmail={mustVerifyEmail} />
                )}
                {activeTab === 'security' && (
...
function AccountInfoTab({ user, student, settings, currencies, mustVerifyEmail }: { user: Props['user']; student: Props['student']; settings: Props['settings']; currencies: Currency[]; mustVerifyEmail: boolean }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put('/student/settings/account');
    };

    const handleResendVerification = () => {
        verificationForm.post('/student/settings/resend-verification');
    };
                            ...
                            <Select value={data.gender} onValueChange={(value) => setData('gender', value)}>
                                <SelectTrigger className="bg-[#f4f4fa] border border-[#caced7] rounded-[12px] h-auto px-5 py-3 font-['Nunito'] font-light text-[14px] text-[#6b7280]">
                                    <div className="flex items-center gap-3">
                                        <Icon icon="mdi:account-outline" className="w-[18px] h-[18px] text-[#6b7280]" />
...
function SecuritySettingsTab({ user, settings, requiresConfirmation }: { user: Props['user']; settings: Props['settings']; requiresConfirmation: boolean }) {
    ...
    const handlePasswordSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        passwordForm.put('/student/settings/password', {
    ...
    const handleToggleDeactivation = () => {
        ...
        if (confirm(message)) {
            deactivationForm.post('/student/settings/deactivation');
        }
    };

    const handleDeleteAccount = (e: React.FormEvent) => {
        e.preventDefault();
        deleteForm.delete('/student/settings/delete-account', {
...
function NotificationSettingsTab({ settings }: { settings: Props['settings'] }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put('/student/settings/notifications');
    };
...
SettingsIndex.layout = (page: React.ReactNode) => <StudentLayout children={page} />;
