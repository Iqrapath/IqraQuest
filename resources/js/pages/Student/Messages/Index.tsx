...
export default function MessagesIndex({ conversations: initialConversations, selectedConversation: initialSelected, messages: initialMessages }: Props) {
    ...
    const baseUrl = '/student/messages';
    ...
    useEffect(() => {
        ...
        if (presenceChannel) {
            presenceChannel
                .here((users: any[]) => {
                    // Check if other user is in the channel
        ...
        if (channel) {
            channel
                .listen('.message.sent', (data: any) => {
                    ...
                    if (!isFromMe) {
                        setMessages((prev) => {
                            ...
                            // Add the message with is_mine set correctly
                            return [...prev, { ...data, is_mine: false }];
                        });
                    }
                    
                    // Update conversation list (only for messages from others)
                    if (!isFromMe) {
                ...
                .listen('.user.typing', (data: any) => {
                    ...
                    if (data.user.id !== selectedConversation.other_user.id) {
                        return; // Ignore typing from self
                    }
    ...
    const handleSendMessage = async (content: string, file?: File) => {
        ...
        try {
            ...
            // Add message to list (this is the only place we add our own messages)
            setMessages((prev) => {
    ...
    const handleTyping = useCallback(() => {
        ...
        // Throttle typing notifications (max once per second)
        const now = Date.now();
        if (now - lastTypingNotifyRef.current < 1000) {
            return;
        }
        lastTypingNotifyRef.current = now;
        
        axios.post(`${baseUrl}/${selectedConversation.id}/typing`, { is_typing: true })
            .catch(() => {}); // Ignore errors for typing indicator
...
MessagesIndex.layout = (page: React.ReactNode) => <StudentLayout children={page} />;
