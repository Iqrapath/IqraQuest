...
function AccountInfoTab({ user, settings, currencies, mustVerifyEmail }: { user: Props['user']; settings: Props['settings']; currencies: Currency[]; mustVerifyEmail: boolean }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put('/teacher/settings/account');
    };

    const handleResendVerification = () => {
        verificationForm.post('/teacher/settings/resend-verification');
    };
...
function SecuritySettingsTab({ user, settings, requiresConfirmation }: { user: Props['user']; settings: Props['settings']; requiresConfirmation: boolean }) {
    ...
    const handlePasswordSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        passwordForm.put('/teacher/settings/password', {
    ...
    const handleToggleDeactivation = () => {
        ...
        if (confirm(message)) {
            deactivationForm.post('/teacher/settings/deactivation');
        }
    };

    const handleDeleteAccount = (e: React.FormEvent) => {
        e.preventDefault();
        deleteForm.delete('/teacher/settings/delete-account', {
            ...
            <div className="flex items-start justify-between py-4 border-b border-dashed border-gray-300">
                <div className="flex flex-col gap-3 max-w-[400px]">
                    ...
                    <ul className="list-disc list-inside text-[12px] text-[#6b7280] font-['Nunito'] space-y-1">
                        <li>Your profile won't be visible to students</li>
                    ...
                    <div className="flex flex-col gap-3 max-w-[400px]">
                        ...
                        <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3">
                            ...
                            <ul className="list-disc list-inside text-[11px] text-red-600 font-['Nunito'] space-y-0.5">
                                ...
                                <li>You will lose access to all sessions and financial history</li>
...
function NotificationSettingsTab({ settings }: { settings: Props['settings'] }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put('/teacher/settings/notifications');
    };

    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            {/* Customize Alerts Section - Teacher Only */}
            <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center justify-between pl-4 border-l-2 border-gray-100">
                    <div className="flex flex-col gap-0.5">
                        <p className="font-['Nunito'] font-medium text-[14px] text-[#111928]">
                            New Messages
                        </p>
                        <p className="font-['Nunito'] font-light text-[11px] text-[#6b7280]">
                            When students or guardians message you
                        </p>
                    </div>
                    <Checkbox
                        checked={data.alert_new_messages}
                        onChange={(checked) => setData('alert_new_messages', checked)}
                    />
                </div>
                ...
                <div className="flex items-center justify-between pl-4 border-l-2 border-gray-100">
                    <div className="flex flex-col gap-0.5">
                        ...
                        <p className="font-['Nunito'] font-light text-[11px] text-[#6b7280]">
                            When you receive new booking requests
                        </p>
                    </div>
                    <Checkbox
                        checked={data.alert_session_requests}
                        onChange={(checked) => setData('alert_session_requests', checked)}
                    />
                </div>
                ...
                <div className="flex items-center justify-between pl-4 border-l-2 border-gray-100">
                    <div className="flex flex-col gap-0.5">
                        ...
                        <p className="font-['Nunito'] font-light text-[11px] text-[#6b7280]">
                            When funds are released or added to your wallet
                        </p>
                    </div>
                    <Checkbox
                        checked={data.alert_payment_updates}
                        onChange={(checked) => setData('alert_payment_updates', checked)}
                    />
                </div>
            </div>
...
SettingsIndex.layout = (page: React.ReactNode) => <TeacherLayout children={page} />;
