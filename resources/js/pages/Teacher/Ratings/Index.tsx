...
export default function RatingsDashboard({ pendingBookings, recentReviews, stats }: Props) {
    ...
    return (
        <TeacherLayout>
            <Head title="Rating & Feedback - Teacher Dashboard" />
            ...
            <div className="flex flex-col gap-8 p-4 md:p-6 max-w-[800px] mx-auto w-full">
                ...
                <div className="bg-gradient-to-r from-transparent to-[#fffbeb] rounded-full p-8 flex flex-col items-center text-center">
                    <h2 className="font-['Nunito'] font-bold text-[20px] text-[#1a1d56] mb-4">Overall Student Rating</h2>
                    ...
                    {/* Progress Bar */}
                    <div className="w-full max-w-[400px] h-3 bg-white border border-[#e5e7eb] rounded-full overflow-hidden mb-2">
                        <div
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-4">
                            <Link href="/teacher/ratings/feedback" className="font-['Nunito'] font-medium text-[14px] text-[#338078] hover:underline">
                                View Feedback Summary
                            </Link>
                            <Link href="/teacher/ratings/all" className="font-['Nunito'] font-medium text-[14px] text-[#338078] hover:underline">
                                View All
                            </Link>
                        </div>
                    </div>
                            ...
                            <div key={review.id} className="bg-white border border-[#e5e7eb] rounded-[20px] p-6 flex gap-4 shadow-sm">
                                ...
                                <div className="flex-1">
                                    <div className="flex justify-between items-start">
                                        <h4 className="font-['Nunito'] font-bold text-[17px] text-[#1a1d56]">{review.user.name}</h4>
                                        <Link href="/teacher/ratings/feedback" className="text-[#338078] text-[14px] font-medium hover:underline">Reply</Link>
                                    </div>
                ...
                {/* Section 3: Pending Ratings */}
                <div className="flex flex-col gap-4">
                    <h2 className="font-['Nunito'] font-bold text-[20px] text-[#1a1d56]">Students to Rate:</h2>
                        ...
                        <div key={booking.id} className="bg-[#fffbeb] rounded-[24px] p-8 flex flex-col gap-6 shadow-sm border border-[#fef3c7]">
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    <span className="text-[#6b7280] text-[15px]">Student:</span>
                                    <div className="flex items-center gap-2">
                                        <img
                                            src={booking.student.avatar ? `/storage/${booking.student.avatar}` : `https://ui-avatars.com/api/?name=${booking.student.name}&background=338078&color=fff`}
                                            alt={booking.student.name}
                                            className="w-10 h-10 rounded-full object-cover"
                                        />
                                        <span className="text-[#1a1d56] font-bold text-[17px]">{booking.student.name}</span>
                                    </div>
                                </div>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="bg-white border border-[#e5e7eb] rounded-[24px] p-8 flex flex-col gap-4 shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[#6b7280] text-[15px]">Classes Conducted:</span>
                            <span className="text-[#1a1d56] font-bold text-[16px] ml-auto">{stats.totalReviews}</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <Icon icon="mdi:checkbox-marked" className="w-[18px] h-[18px] text-[#338078]" />
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[#1a1d56] font-bold text-[16px] ml-auto">
                                {stats.positiveFeedbackRate}% ({stats.totalReviews > 0 ? `${Math.round(stats.totalReviews * stats.positiveFeedbackRate / 100)} out of ${stats.totalReviews}` : '0 out of 0'} reviews rated 4+ stars)
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <Dialog open={!!selectedBooking} onOpenChange={(open) => !open && setSelectedBooking(null)}>
                <DialogContent className="sm:max-w-[480px] bg-white rounded-[24px] p-6 shadow-2xl border-none">
                    ...
                    {selectedBooking && (
                        <div className="flex flex-col gap-5 py-4">
                            <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-xl">
                                <img
                                    src={selectedBooking.student.avatar ? `/storage/${selectedBooking.student.avatar}` : `https://ui-avatars.com/api/?name=${selectedBooking.student.name}&background=338078&color=fff`}
                                    alt={selectedBooking.student.name}
                                    className="w-14 h-14 rounded-full object-cover border-2 border-white shadow-sm"
                                />
                                <div>
                                    <p className="font-['Nunito'] font-bold text-[17px] text-[#1a1d56]">{selectedBooking.student.name}</p>
                                    <p className="font-['Nunito'] text-[14px] text-[#338078]">{selectedBooking.subject.name}</p>
                                </div>
                            </div>

                            <div className="flex flex-col gap-3 items-center py-2">
                                <p className="font-['Nunito'] font-semibold text-[15px] text-[#333]">How was the student's performance?</p>
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="font-['Nunito'] font-semibold text-[14px] text-[#333]">Private Feedback for Student</label>
                                <Textarea
                                    placeholder="Tell the student how they did and what to improve..."
                                    value={data.comment}
                    ...
                    <DialogFooter>
                        ...
                        >
                            {processing ? 'Sending Feedback...' : 'Send Feedback'}
                        </button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
