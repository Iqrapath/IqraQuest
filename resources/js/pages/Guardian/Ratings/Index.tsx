...
interface Review {
    ...
    user: { name: string }; // The student's name
    rating: number;
    comment: string;
    created_at: string;
...
interface Booking {
    id: number;
    teacher: { user: { name: string; avatar: string | null } };
    student: { name: string }; // The student's user object
    subject: { name: string };
    end_time: string;
}
        ...
        <GuardianLayout>
            ...
            <div className="flex flex-col gap-8 p-4 md:p-6 max-w-[800px] mx-auto w-full">
                ...
                <div className="bg-gradient-to-r from-transparent to-[#fffbeb] rounded-full p-8 flex flex-col items-center text-center">
                    <h2 className="font-['Nunito'] font-bold text-[20px] text-[#1a1d56] mb-4">Overall Children Rating</h2>
                    ...
                    <p className="font-['Nunito'] text-[15px] text-[#6b7280] mb-6">
                        Based on {stats.totalReviews} teacher reviews for your children
                    </p>
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-4">
                            <Link href="/guardian/ratings/feedback" className="font-['Nunito'] font-medium text-[14px] text-[#338078] hover:underline">
                                View Feedback Summary
                            </Link>
                            <Link href="/guardian/ratings/all" className="font-['Nunito'] font-medium text-[14px] text-[#338078] hover:underline">
                                View All
                            </Link>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        {recentReviews.length > 0 ? recentReviews.map(review => (
                            <div key={review.id} className="bg-white border border-[#e5e7eb] rounded-[20px] p-6 flex gap-4 shadow-sm">
                                ...
                                <div className="flex-1">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <Link href="/guardian/ratings/feedback" className="text-[#338078] text-[14px] font-medium hover:underline">Reply</Link>
                                    </div>
                            ...
                            <p className="text-gray-400 font-['Nunito'] italic text-center py-4">No reviews yet for your students.</p>
                        )}
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-4">
                    <h2 className="font-['Nunito'] font-bold text-[20px] text-[#1a1d56]">Pending Ratings (Children):</h2>
                    {pendingBookings.length > 0 ? pendingBookings.map(booking => (
                        <div key={booking.id} className="bg-[#fffbeb] rounded-[24px] p-8 flex flex-col gap-6 shadow-sm border border-[#fef3c7]">
                            <div className="flex flex-col gap-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[#338078] font-bold text-[17px]">{booking.student.name}</span>
                                </div>
                                <div className="flex items-center gap-2">
                        ...
                        <p className="text-gray-400 font-['Nunito'] italic text-center py-4 bg-white border border-dashed rounded-xl">No pending ratings for your children.</p>
                    )}
                </div>

                {/* Section 4: Performance Summary */}
                <div className="flex flex-col gap-4">
                    <h2 className="font-['Nunito'] font-bold text-[20px] text-[#1a1d56]">Performance Summary (All Children)</h2>
                    <div className="bg-white border border-[#e5e7eb] rounded-[24px] p-8 flex flex-col gap-4 shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[#6b7280] text-[15px]">Average Response Time:</span>
                            <span className="text-[#1a1d56] font-bold text-[16px] ml-auto">{stats.responseTime}</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <Icon icon="mdi:checkbox-marked" className="w-[18px] h-[18px] text-[#338078]" />
                            <span className="text-[#6b7280] text-[15px]">Overall Attendance Rate:</span>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[#1a1d56] font-bold text-[16px] ml-auto">
                                {stats.positiveFeedbackRate}% ({stats.totalReviews > 0 ? `${Math.round(stats.totalReviews * stats.positiveFeedbackRate / 100)} out of ${stats.totalReviews}` : '0 out of 0'} teacher reviews rated 4+ stars)
                            </span>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="flex flex-col gap-5 py-4">
                            <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-xl">
                                ...
                                <div>
                                    ...
                                    <p className="font-['Nunito'] text-[12px] text-[#666]">Student: {selectedBooking.student.name}</p>
                                </div>
                            </div>

                            <div className="flex flex-col gap-3 items-center py-2">
                                <p className="font-['Nunito'] font-semibold text-[15px] text-[#333]">How was the experience?</p>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <Textarea
                                    placeholder="Share your feedback about this Session for your student..."
                                    value={data.comment}
