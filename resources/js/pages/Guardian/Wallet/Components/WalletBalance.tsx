...
export const WalletBalance: React.FC<WalletBalanceProps> = ({ balance, paystackPublicKey }) => {
    ...
    const handleConfirmPayment = async (channel: string) => {
        ...
        try {
            ...
            const response = await axios.post('/guardian/payment/initialize', payload);
    ...
    return (
        <div className={`bg-white rounded-[30px] p-6 md:p-8 shadow-sm relative overflow-hidden transition-all duration-300 ${showTopUp ? 'min-h-[365px]' : ''}`}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                ...
                <Link href="/guardian/wallet/transactions" className="text-[#2D7A70] hover:underline text-sm font-medium hidden md:block">
                    Check Payment History
                </Link>
                {/* Mobile only link */}
                <Link href="/guardian/wallet/transactions" className="text-[#2D7A70] hover:underline text-sm font-medium md:hidden w-full text-center py-2 bg-gray-50 rounded-lg">
                    Check Payment History
                </Link>
            </div>
