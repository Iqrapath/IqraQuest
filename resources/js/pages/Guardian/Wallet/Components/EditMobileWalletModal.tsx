...
interface EditMobileWalletModalProps {
    isOpen: boolean;
    onClose: () => void;
    walletDetails: {
        id: number;
...
export const EditMobileWalletModal: React.FC<EditMobileWalletModalProps> = ({
    isOpen,
    onClose,
    walletDetails,
}) => {
    ...
    // Set initial values when walletDetails changes
    useEffect(() => {
        if (walletDetails) {
            setPhoneNumber(walletDetails.wallet_phone_number);
            setSelectedProvider(walletDetails.wallet_provider);
            setAccountName(walletDetails.wallet_account_name);
        }
    }, [walletDetails]);
    ...
    useEffect(() => {
        if (!isOpen) {
            ...
            // Re-populate if reopening with same details
            setPhoneNumber(walletDetails.wallet_phone_number);
            setSelectedProvider(walletDetails.wallet_provider);
            setAccountName(walletDetails.wallet_account_name);
        }
    ...
    const handleSubmit = async () => {
        if (!isValid || !accountName || !walletDetails) {
        ...
        try {
            const response = await fetch(`/guardian/payment/methods/mobile-wallet/${walletDetails.id}`, {
                method: 'PUT',
                headers: {
            ...
            if (response.ok) {
                toast.success('Mobile wallet updated successfully!');
                onClose();
                window.location.reload();
            } else {
                toast.error(data.message || 'Failed to update mobile wallet');
            }
        } catch (error) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="w-[95vw] sm:w-full sm:max-w-[600px] p-0 gap-0 bg-white border-none shadow-xl overflow-hidden rounded-[26px]">
                <VisuallyHidden>
                    <DialogTitle>Edit Mobile Wallet</DialogTitle>
                    <DialogDescription>Edit your mobile money wallet details.</DialogDescription>
                </VisuallyHidden>
