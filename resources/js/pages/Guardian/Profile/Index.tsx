...
export default function ProfileIndex({ auth, guardian, subjects, flash }: Props) {
    const { user } = auth;
    ...
    const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            postAvatar('/guardian/profile/avatar');
        }
    };

    // Helper to format days for display
    const getFormattedDays = () => {
        if (!guardian.preferred_days) return [];
        if (Array.isArray(guardian.preferred_days)) {
            // Check if objects or strings
            if (guardian.preferred_days.length > 0 && typeof guardian.preferred_days[0] === 'object') {
                return guardian.preferred_days.map((d: any) => d.day);
            }
            return guardian.preferred_days;
        }
        return [];
    };
                        ...
                        <div className="flex items-center gap-4 mt-1">
                            <div className="flex items-center gap-1 text-[#338078]">
                                ...
                                <span className="font-['Nunito'] text-[14px]">
                                    {guardian.city && guardian.country ? `${guardian.city}, ${guardian.country}` : 'Location not set'}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <span className="font-['Nunito'] font-medium text-[16px] text-[#111928]">
                                {guardian.city && guardian.country ? `${guardian.city}, ${guardian.country}` : '-'}
                            </span>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            <div className="flex flex-col gap-1">
                                ...
                                <span className="font-['Nunito'] font-medium text-[16px] text-[#111928]">{guardian.availability_type || '-'}</span>
                            </div>
                            <div className="flex flex-col gap-1">
                                <span className="font-['Nunito'] text-[12px] text-[#9ca3af]">Time Zone:</span>
                                <span className="font-['Nunito'] font-medium text-[16px] text-[#111928]">{guardian.timezone || '-'}</span>
                            </div>
                        </div>
                    </div>
                </div>

                {/* Learning Preferences Section for Guardian */}
                <div className="bg-white rounded-[20px] shadow-[0px_0px_15px_0px_rgba(68,68,68,0.1)] p-8 relative">
                    <div className="flex justify-between items-start mb-6">
                        <h3 className="font-['Nunito'] font-medium text-[20px] text-[#111928]">
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex flex-wrap gap-6">
                                {subjects.map(subject => {
                                    const selected = guardian.subjects.map((s: any) => s.id).includes(subject.id);
                                    return (
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <span className="font-['Nunito'] font-medium text-[16px] text-[#111928]">
                                {guardian.learning_goal_description || 'No goal set'}
                            </span>
                        </div>
                    </div>
                </div>
...
ProfileIndex.layout = (page: React.ReactNode) => <GuardianLayout children={page} />;
