...
export default function EditPreferencesModal({ open, onOpenChange, guardian, subjects }: Props) {
    const { data, setData, post, processing } = useForm({
        subjects: [] as number[],
        learning_goal_description: guardian.learning_goal_description || '',
    });

    useEffect(() => {
        if (open) {
            setData({
                subjects: guardian.subjects.map((s: any) => s.id),
                learning_goal_description: guardian.learning_goal_description || '',
            });
        }
    }, [open, guardian]);
