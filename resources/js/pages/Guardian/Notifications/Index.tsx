...
export default function NotificationsIndex({ paginatedNotifications, paymentTypes, unreadCount }: Props) {
    ...
    const handleMarkAsRead = (id: string) => {
        setProcessingId(id);
        router.post(`/guardian/notifications/${id}/read`, {}, {
            preserveScroll: true,
    ...
    const handleMarkAllAsRead = () => {
        router.post('/guardian/notifications/mark-all-read', {}, {
            preserveScroll: true,
            only: ['paginatedNotifications', 'unreadCount'],
        });
    };
    ...
    const getNotificationIcon = (notification: Notification) => {
        ...
        if (type.includes('booking') || type.includes('request') || type.includes('session') || type.includes('class')) {
            return { icon: 'mdi:bell-outline', color: 'text-blue-500', bg: 'bg-blue-50' };
        }
    ...
    const hasActions = (notification: Notification) => {
        const type = notification.data.type || notification.type;
        // Guardians only have Reply action for messages, not Accept/Decline for bookings
                ...
                <div className="bg-white rounded-[22px] shadow-[0px_0px_11px_0px_rgba(51,128,120,0.05)] p-5">
                    {filteredNotifications.length === 0 ? (
                        ...
                        <div className="space-y-4">
                            {filteredNotifications.map((notification) => {
                                ...
                                const showActions = hasActions(notification);
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-start gap-4">
                                            ...
                                            {/* Action Buttons - Only Reply for messages (Guardians don't have Accept/Decline) */}
                                            {showActions && notification.data.type?.includes('message') && (
                    ...
                    {paginatedNotifications.last_page > 1 && (
                        <div className="flex items-center justify-center gap-2 mt-6 pt-4 border-t border-gray-100">
                            <button
                                onClick={() => router.get(`/guardian/notifications?page=${paginatedNotifications.current_page - 1}`)}
...
NotificationsIndex.layout = (page: React.ReactNode) => <GuardianLayout children={page} />;
